...
public class RigidBodyController : MonoBehaviour
{
    ...
    public void Locomote(Vector3 direction)
    {
        ...
        direction.z *= Mathf.Cos(boatOffset * Mathf.Deg2Rad) * speed * Time.deltaTime;
        direction.x *= Mathf.Cos(boatOffset * Mathf.Deg2Rad) * speed * Time.deltaTime;
    ...
    public void Rotate()
    {
        ...
        if (cam.GetCamMode() == 1)
        {
            cam.gameObject.transform.rotation = Quaternion.Euler(rotateXAxis, rotateYAxis + boatOffset, 0);
        }
        else
        {
